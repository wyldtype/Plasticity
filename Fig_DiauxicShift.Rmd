---
title: "Level and plasticity divergence during the diauxic shift"
---
...
# Scer gene counts:
expdf |> dplyr::select(cer) |> table()
expdf |> dplyr::select(par) |> table()
...
expdf |> filter(plasticity != "diverged" & 
  ...
  mutate(direction = if_else(effect_size_species > 0,
                             true = "up in Scer",
                             false = "up in Spar")) |> 
  dplyr::select(cer, par, direction) |> table()

cat("Diverged plasticity:")
expdf |> filter(plasticity == "diverged" & 
                  level != "diverged") |> 
  mutate(isZero = if_else(cer == 0 | par == 0,
                          true = if_else(cer == 0,
                                         ...
                                         false = "Spar0"),
                          false = "neither0")) |> 
  dplyr::select(cer, par, isZero) |> table()

cat("Diverged level and plasticity:")
expdf |> filter(plasticity == "diverged" & 
                  level == "diverged") |> 
  mutate(direction = if_else(effect_size_species > 0,
                             true = "up in Scer",
...
cluster_names <- expdf |> filter(plasticity != "diverged" & 
                                   level != "diverged") |> 
  dplyr::select(cer, par) |> pull() |> unique()
...
print(annotate_figure(plotGenes(.gene_idxs = gene_idxs, .quartet = FALSE,
                                .experiment_name = experiment_name,
                                .plotlims = c(-2.1, 2.1), .normalization = "scale") +
                        theme(plot.margin = unit(c(0,0.25,0,0), "cm")),
                      top = paste0(length(gene_idxs), " genes")))
dev.off()
...
prop_table <- expdf |> mutate(plasticity_diverged = plasticity == "diverged",
  ...
  dplyr::select(plasticity_diverged, level_diverged) |> table()
prop_table
sum(prop_table)
...
if (experiment_name == "HAP4") {
  ...
  # level
  expdf |> filter(level == "diverged" & plasticity == "conserved") |> dplyr::select(gene_name) |> 
    pull() |> sample(1)
  # we chose: YBR067C
  ...
  # plasticity
  expdf |> filter(level == "conserved" & plasticity == "diverged") |> dplyr::select(gene_name) |> 
