---
title: "Comparing results using different numbers of clusters and low-variability thresholds"
---
...
2) a user-specified threshold for miniumum variability a gene can exhibit to be considered to have expression plasticity

...influence which genes are called as diverging in plasticity between species.

```{r package-loading, include=FALSE}
# library(ComplexHeatmap) #TODO: complexheatmap not available anymore, remove dependencies
sapply(c("WGCNA", "stringr", "ggplot2", "RColorBrewer", "ggpubr", "grid", "circlize", "huxtable"), require, character.only=TRUE)
source("functions_for_figure_scripts.R")
load("data_files/QC_CorrelationClustering.RData")
load("data_files/FinalDataframe_SNR1.RData")
...
clustdf <- dplyr::select(finaldf, gene_name, experiment, level, cer, par) |>
...
getClusterQCHeatmap <- function(.e) {
  plot_cer <- clustdf |> filter(experiment == .e) |>
    ...
    dplyr::select(gene_ID, level, nclust, cer)
  ...
  plot_par <- clustdf |> filter(experiment == .e) |>
    ...
    dplyr::select(gene_ID, level, nclust, par)
  ...
  plot_mat_e <- plotdf |> dplyr::select(gene_ID, nclust, group4) |>
...
```{r dispdf}
dispdf <- dplyr::select(finaldf, gene_name, experiment, level, cer, par) |>
...
getDispersionQCHeatmap <- function(.e) {
  plot_cer <- dispdf |> filter(experiment == .e) |>
    ...
    dplyr::select(gene_ID, level, dispersion, cer)
  ...
  plot_par <- dispdf |> filter(experiment == .e) |>
    ...
    dplyr::select(gene_ID, level, dispersion, par)
  ...
  plot_mat_e <- plotdf |> dplyr::select(gene_ID, dispersion, group4) |>
...
p <- ggplot(plotdf, aes(x = cer, y = par, fill = type)) +
  ...
  geom_text(aes(label = pct,
                color = grepl(pattern = "conserved", x = type)), size = 2) +
...
And total nGenes for paper figure:

```{r sometimes-static-3disp-ngenes}
plotdf_sums
```
...
finaldf_1disp <- dplyr::select(finaldf, -cer, -par) |> 
...
```{r sometimes-static-1disp-ngenes}
plotdf_sums
```
...
finaldf_5disp <- dplyr::select(finaldf, -cer, -par) |> 
...
```{r sometimes-static-5disp-ngenes}
plotdf_sums
```
