...
load("data_files/CorrelationClustering_SNR30pct.RData")

# setting random seed for statistical test null samples
set.seed(23)
...
ggarrange(plotlist = unlist(plotlist), nrow = length(ExperimentNames), 
          ncol = length(ExperimentNames))
ggsave(file = "paper_figures/ExperimentOverview/distribution_mat_upSpar.pdf", width = 15, height = 15)
```
...
#           grid.text(sprintf("%.2f", plot_mat[i, j]), x, y, gp = gpar(fontsize = 10))
#         }, show_heatmap_legend = FALSE)
p <- pheatmap(-plot_mat, cluster_rows = FALSE, cluster_cols = FALSE)
p
...
for (e_detect in ExperimentNames) {
  ...
  for (e_measure in ExperimentNames) {
    ...
    # uncomment below line to have a binary threshold of leq 0 versus greater than 0
    #e_cors <- as.numeric(e_cors <= 0)
...
for (e_detect in names(plotlist)) {
  ...
  #               show_heatmap_legend = FALSE,
  #               na_col = "grey30"))
  print(pheatmap(plotlist[[e_detect]], cluster_rows = FALSE, cluster_cols = FALSE))
  #print(plotlist_names[[e_detect]]) # to ID specific genes of interest
}

# TODO: looks like some sort of scaling to control for how certain environments
# have higher correlations overall would help show the pattern more
```
