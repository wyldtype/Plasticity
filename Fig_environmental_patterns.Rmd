...
Fisher exact statistical tests for plasticity divergence, any type of plasticity divergence. One-sided test b/c we only want to test if diverged-plasticity genes are MORE likely than the rest of genes to be diverging in plasticity in other environments.

```{r fisher-test-matrix}
plotlist <- purrr::map(ExperimentNames, \(e) {
  outlist <- vector(mode = "list", length = length(ExperimentNames))
  names(outlist) <- ExperimentNames
  return(outlist)
...
for (e_row in ExperimentNames) {
  for (e_col in ExperimentNames) {
    ...
    e_gene_idxs_diverged <- finaldf |> filter(experiment == e_row &
                                                plasticity == "diverged") |> 
      dplyr::select(gene_name) |> pull()
    # set of non-upSpar genes in home environment (not necessarily same size):
    e_gene_idxs_conserved <- finaldf |> filter(experiment == e_row &
...
ggarrange(plotlist = unlist(plotlist), nrow = length(ExperimentNames), 
          ncol = length(ExperimentNames))
ggsave(file = "paper_figures/EnvironmentalPatterns/plasticity_fisher_mat.pdf", width = 15, height = 15)
```

Fisher exact statistical tests for 1-2 plasticity divergence versus a same-sized sample of all other genes:
