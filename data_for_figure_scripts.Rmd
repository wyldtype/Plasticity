...
Selecting which clustering scheme to use for finaldf
```{r select-clustering-df}
finaldf_filename <- "data_files/FinalDataframe_SNR30pct.RData"
clusterdf <- clusterdf_snr30pct
```
...
finaldf |> filter(plasticity == "diverged") |> dplyr::select(all_of("experiment")) |> table()
finaldf |> filter(level == "diverged") |> dplyr::select(all_of("experiment")) |> table()
```

Tables of number of timepoints per experiment for paper figure:

```{r paper-table-samples}
sample_info |> dplyr::select(all_of(c("experiment", "time_point_str"))) |> 
  unique() |> 
  group_by(experiment) |> 
  summarise(n = n())
```
...
test_idx_conserved <- finaldf |> filter(plasticity == "conserved" & cor_parents > .5) |> dplyr::select(all_of(c("gene_name", "experiment"))) |> slice_sample(n = 1)
test_common_cols <- info |> filter(experiment == test_idx_conserved$experiment) |> dplyr::select(all_of("condition")) |> pull()
...
test_idx_diverged <- finaldf |> filter(plasticity == "diverged" & cor_parents < -.5) |> dplyr::select(all_of(c("gene_name", "experiment"))) |> slice_sample(n = 1)
test_common_cols <- info |> filter(experiment == test_idx_diverged$experiment) |> dplyr::select(all_of("condition")) |> pull()
...
p_none <- plotdf |> filter(bias == "none") |> 
  dplyr::select(all_of(c("cer", "par"))) |> table()
...
finaldf <- filter(finaldf, eff_threshold == "level") |> 
  dplyr::select(-all_of("eff_threshold"))
```
...
save(finaldf, colordf, ExperimentNames,
     LongExperimentNames, file = finaldf_filename)
```
...
supp_tab1 <- finaldf |> dplyr::select(all_of(c("gene_name", "experiment", "effect_size_species", "padj_species", "cer", "par", "level", "plasticity"))) |>
...
rm(supp_tab1)
supp_tab2 <- finaldf |> dplyr::select(all_of(c("gene_name", "experiment", "effect_size_species", "padj_species", "effect_size_allele", "padj_allele", "cor_hybrid", "cor_parents", "ABC_hybrid", "ABC_parents", "cer", "par", "level", "plasticity"))) |>
