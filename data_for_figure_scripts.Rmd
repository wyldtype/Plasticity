...
Adding hybrid clustering results for cis/trans test:

```{r hybrid-clustering}
finaldf <- clusterdf_hyb |> dplyr::rename(c("gene_name"="gene_ID", "hyc"="cer", "hyp"="par")) |> 
  right_join(y = finaldf, by = c("experiment", "gene_name"))
```
...
finaldf |> group_by(experiment) |> 
  summarise(n_cer_lowvar = sum(cer == 0),
            ...
            n_hyc_lowvar = sum(hyc == 0),
            n_hyp_lowvar = sum(hyp == 0),
            n_genes = n()*2) |> 
