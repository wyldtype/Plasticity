...
The actual version of these functions that is sourced in the other .Rmd scripts is functions_for_figure_scripts.R. This is the webpage-ready version displaying code for each function with accompanying unit tests
...
print("LowPi, shouldn't have changed much")
print("only a couple samples have replicates, and only in par")
sample_info |> filter(experiment == "LowPi" & organism == "par") |>
  dplyr::select(condition) |> table() |> sort(decresing = TRUE)
print("cer first, should not have changed")
...
print("now par, also should not have changed")
test1 <- counts[,sample_info$experiment == "LowPi" &
...
dim(cbind(test_collapse$cer, test_collapse$par))
print("HAP4 in hyc, should not have changed")
...
plotExpressionProfilePairBasic <- function(.cts1, .cts2, 
  ...
  if (.normalization == "none" | .normalization == "scale") {
    ...
    if (.confidence_type == "all") { # standard deviation
...
plotExpressionProfilePair(collapsed$par["YHL006C",, drop = FALSE],
                          ...
                          .color2 = "red")
```
```{r plotExpressionProfileQuartetBasic}
