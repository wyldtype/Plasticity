...
## Reading in Krieger et al. 2020 tagseq data
...
tagseq <- list.files("data_files/tagseq_counts/", full.names = TRUE) |> 
  ...
  map(.f = dplyr::select, X1, X3) |> # X1 are gene names, X3 is the sense strand read count
...
Do any genes have consistently biased mappings? These might be genes with above-average sequence conservation between Scer and Spar and therefore shouldn't be used for allele-specific comparisons in the hybrid

```{r identifying-biased-genes}
...
par_genedf <- plotdf |> filter(organism == "parSample") |> 
  dplyr::group_by(gene) |> 
  summarise(avg_pct_cer = mean(pct_cer, na.rm = TRUE))
cer_biased_genes <- par_genedf |> filter(avg_pct_cer > 0.1) |> dplyr::select(gene) |> pull()
par_biased_genes <- cer_genedf |> filter(avg_pct_cer < 0.9) |> dplyr::select(gene) |> pull()
...
For simplicity, we are measuring all gene allele mappings as "percent of reads mapping to the S.cer allele of the gene". We expect genes from Scer parental samples to have close to 100% of reads mapping to the Scer allele. We expect genes from Spar parental samples to have close to 0% of reads mapping to the Scer allele. The threshold of 10% of reads mapping to wrong species appears to be a good cutoff for identifying genes with biased mappings. These do tend to be genes that are on the lower end of expression level.

What are the common names of these genes?

```{r biased-common-names}
# any common genes of note?
sgd_lookup <- read_tsv("data_files/downloaded_genomes_and_features/SGD_features.tab",
                       col_names = FALSE) |> 
  dplyr::select(X4, X5) |> unique() |> drop_na() |> 
  dplyr::rename(c("systematic"="X4", "common"="X5"))
# cer biased
par_genedf |> filter(gene %in% cer_biased_genes) |> 
  left_join(y = sgd_lookup, by = c("gene"="systematic")) |> 
...
# both
bind_rows(cer_genedf |> filter(gene %in% both_biased_genes) |> 
            arrange(avg_pct_cer),
          par_genedf |> filter(gene %in% both_biased_genes) |> 
            arrange(desc(avg_pct_cer))) |> 
  left_join(y = sgd_lookup, by = c("gene"="systematic")) |> 
  print(n = length(both_biased_genes)*2)
```

As Fay et al. 2023 data does not have separate parental samples, we will record all the based genes and remove them after adding Fay et al. 2023 samples and filtering for low expression.
...
fay <- list.files("data_files/fay_counts/", full.names = TRUE) |> 
  ...
  map2(.y = fay_filenames,
       .f = \(x, y) {
         if (grepl("R1", y)) {
           output <- dplyr::select(x, X1, X2)
           return(output)}
         if (grepl("R2", y)) {
           output <- dplyr::select(x, X1, X3)
           return(output)}
...
sampdf <- fay_sampdf |> 
  dplyr::select(Sample, Sc_gene_reads, Sp_gene_reads) |>
...
tagseq_cer <- tagseq_cer |> dplyr::select(-gene) |> as.matrix()
tagseq_par <- tagseq_par |> dplyr::select(-gene) |> as.matrix()
...
info_fay <- read.xlsx("data_files/downloaded_from_Fay2023/Supporting_Tables.xlsx",
                      sheet = 4, startRow = 2) |> 
  dplyr::select(Sample, Condition, Time, Strains) |> 
...
info_tagseq <- dplyr::select(info_tagseq, -collection_date)
...
info_fay <- dplyr::select(info_fay, -parents_or_hybrid)
...
sample_info |> filter(organism %in% c("cer", "par") & grepl("GS2018", well_flask_ID)) |> 
  dplyr::select(well_flask_ID) |> table() 
...
sample_info$new_id <- map2(sample_info$well_flask_ID, 
             sample_info$sample_name, \(i, s) {
               ex <- sample_info |> filter(sample_name == s) |> 
                 dplyr::select(experiment) |> pull()
               if (ex == "LowN") {
                 org <- sample_info |> filter(sample_name == s) |> 
                   dplyr::select(organism) |> pull()
...
sample_info <- dplyr::select(sample_info, -"new_id")
...
sample_info |> filter(experiment == "CC") |> 
  dplyr::select(time_point_str, time_point_num) |> unique() |> arrange(time_point_num)
...
sample_info %>% filter(experiment == "LowPi") %>% dplyr::select(genotype) %>% table()
# species
...
plotdf <- tibble(sample_name = names(libsizes[grepl("_hy[pc]", names(libsizes))]),
  ...
  left_join(y = dplyr::select(filter(sample_info, organism == "hyb"), sample_name, experiment), by = "sample_name")
...
plotdf <- tibble(libsize = libSizes, 
  ...
  left_join(dplyr::select(sample_info, sample_name, experiment),
            by = "sample_name")
...
gene_lens_scer <- gene_lens_scer |> dplyr::select(gene_name, length) |> 
  filter(gene_name %in% common_genes) |> unique()
gene_lens_spar <- gene_lens_spar |> dplyr::select(gene_name, length) |> 
  filter(gene_name %in% common_genes) |> unique()
...
plotdf <- left_join(x = dplyr::rename(dplyr::select(gene_lens_scer, gene_name, length), "Scer"="length"),
                    y = dplyr::rename(dplyr::select(gene_lens_spar, gene_name, length), "Spar"="length"),
                    by = "gene_name")
...
sample_info |> dplyr::select(sample_name, organism, allele) |> slice_sample(n = 10)
sample_info_allele |> dplyr::select(sample_name, organism, allele) |> slice_sample(n = 10)
# checking that every hybrid sample name has exactly one cer and one par row
sample_info_allele |> dplyr::select(sample_name, organism, allele) |> 
  group_by(sample_name) |> 
  summarise(ncernpar = unique(table(allele))) |> 
  dplyr::select(ncernpar) |> table()
sample_info_allele |> dplyr::select(allele) |> table()
```
...
save(counts, sample_info,
     ...
     par_biased_genes,
     both_biased_genes,
     file = "data_files/Cleaned_Count_Data.RData")
