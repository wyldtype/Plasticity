...
sum(!(rownames(regmats$YPD) %in% colnames(regmats$YPD))) # should be 0
rowSums(regmats$YPD[!(rownames(regmats$YPD) %in% colnames(regmats$YPD)),]) # if not 0, how many total connections?
...
plotdf <- purrr::map2(names(regmats), regmats, \(nm, mat) {
  ...
  right_join(y = finaldf, by = c("target_name"="gene_name",
                                 "experiment")) |> 
  drop_na()
plot_table <- plotdf |> group_by(experiment, plasticity) |> 
  summarise(n = n())
plotdf_downsampled <- plotdf |> group_by(experiment, plasticity) |> 
  slice_sample(n = min(plot_table$n))
ggplot(plotdf_downsampled, aes(x = nRegulators)) +
  # geom_density(aes(fill = plasticity, y = after_stat(count)), 
  #              alpha = 0.25, bw = 1) +
  geom_histogram(aes(fill = plasticity, y = after_stat(count)), position = "dodge",
                 binwidth = 0.5) +
...
### TODO: I accidentally git restored (fml) and removed the chi-sq tests. They're not tricky just testing for chi-sq enrichment among the 6 gene groups that aren't conserved plasticity (i.e. 1-0, 2-1, 2-0, not 2-2, 1-1, 0-0)

```{r chisq, message=FALSE}
if (file.exists("data_files/ChiSq.RData")) {
  load("data_files/ChiSq.RData")
}
chisqdf$adj_pval <- p.adjust(chisqdf$chi_pval, method = "BH")
```

### Supplementary Figure: avg expression of largest gene group for each regulator that was a sig hit in the chisqdf

```{r supp-regulons, warning=FALSE, message=FALSE}
plotdf <- chisqdf |> group_by(tf) |> 
  summarise(nSig = sum(adj_pval < 0.05))
sum(plotdf$nSig != 0)
nrow(plotdf)
sum(plotdf$nSig != 0)/nrow(plotdf) # % of regulators with at least one divergent environment, counting each regulator once
plotdf <- chisqdf |> group_by(tf, home) |> 
  summarise(nSig = sum(adj_pval < 0.05))
sum(plotdf$nSig != 0)
nrow(plotdf)
sum(plotdf$nSig != 0)/nrow(plotdf) # % of regulators with at least one divergent environment, counting regulators with multiple homes multiple times
plotdf_sig <- filter(chisqdf, adj_pval < 0.05) |> 
  dplyr::select(all_of(c("tf", "home"))) |> unique()
plotlist <- map(c(1:nrow(plotdf_sig)), \(i) {
  cat(i, "/", nrow(plotdf_sig), "\n")
  tf <- plotdf_sig$tf[i]
  tf_common <- yeastract_lookup |> filter(systematic == tf) |> 
    dplyr::select(all_of("common")) |> pull()
  e <- plotdf_sig$home[i]
  reg_idxs <- filter(regdf, regulator == tf & environment == e) |>
    dplyr::select(all_of("target")) |> pull()
  countdf <- finaldf |> filter(gene_name %in% reg_idxs) |> 
    dplyr::select(all_of(c("gene_name", "experiment", "cer", "par"))) |> 
    group_by(experiment, cer, par) |> 
    summarise(n = n()) |> filter(cer != par & experiment != e) |> 
    arrange(desc(n))
  cer_clust <- countdf$cer[1]
  par_clust <- countdf$par[1]
  e2 <- countdf$experiment[1]
  regulondf <- regdf |> filter(regulator == tf & environment == e) |> 
    left_join(filter(finaldf, experiment == e2),
              by = c("target"="gene_name"))
  gene_idxs <- regulondf |> filter(cer == cer_clust & par == par_clust) |> 
    dplyr::select(all_of("target")) |> pull()
  p <- plotHomeAndAltEnvironments(.tf_common = tf_common,
                                  .reg_idxs = gene_idxs,
                                  .e = e, .e2 = e2, .plotlims = c(0, 12), .include_hybrid = FALSE)
  return(p)
})
ggarrange(plotlist = plotlist, ncol = 2,
          nrow = ceiling(length(plotlist)/2))
# adding blank plot for key in main figure
p_blank <- ggplot() + theme_void()
plotlist0 <- c(list(p_blank), plotlist)
pdf("paper_figures/Yeastract/Supp_regulons.pdf",
    width = 12, height = 4*ceiling(length(plotlist0)/2))
ggarrange(plotlist = plotlist0, ncol = 2,
          nrow = ceiling(length(plotlist0)/2))
dev.off()
```
