...
plotdf <- purrr::map2(names(regmats), regmats, \(nm, mat) {
  taget_names_diverged <- intersect(x = finaldf$gene_name[finaldf$experiment == nm & finaldf$plasticity == "diverged"], y = colnames(mat))
  taget_names_conserved <- intersect(x = finaldf$gene_name[finaldf$experiment == nm & finaldf$plasticity == "conserved"], y = colnames(mat))
...
### TODO: I accidentally git restored (fml) and removed the histogram we've been using in the main plot. which is not downsampled and only shows genes with at least one regulator


```{r histogram-all-environments}
p <- ggplot(plotdf_downsampled, aes(x = nRegulators)) +
  geom_histogram(aes(fill = plasticity), position = "dodge",
                 binwidth = 0.5) +
  theme_classic() +
  xlab("number of regulators per target gene") +
  scale_x_continuous(breaks = c(0:7)) +
...
### TODO: plot avg expression of largest gene group for each regulator that was a sig hit in the chisqdf
