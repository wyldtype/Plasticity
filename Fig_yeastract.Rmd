...
for (e in ExperimentNames) {
  ...
  fisher_mat <- plotdf_e |> 
    select(nGenes, nInteractions) |> as.matrix()
...
plotdf <- tfdf |> group_by(experiment) |> 
  ...
  rename(c("home"="experiment", "tf"="gene_name"))
...
plotdf <- tfdf |> rename(c("home"="experiment", "tf"="gene_name"))
...
suppTable3 <- tfdf |> rename(c("home"="experiment", "tf"="gene_name", 
  ...
  left_join(y = select(yeastract_lookup, 
                       common, systematic), 
            by = c("tf"="systematic")) |> 
  select(home, tf, common, regulon_size,
