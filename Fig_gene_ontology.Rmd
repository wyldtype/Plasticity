...
## How many genes diverging in plasticity are membrane-bound transporters?

```{r membrane-transporters}
plotdf <- goslim_full |> filter(GO_aspect %in% c("F")) |> 
...
# plotdf$membrane_transporter <- purrr::map2(plotdf$C, plotdf$P, \(comp, proc) {
...
plotGenes(gene_idxs, .experiment_name = "CC") # 1-2 divergers
...
## Regulon-based GO analysis
GO analysis of specific regulons with over 30% diverged plasticity genes (regulons of PDR1, PDR3, RLM1, RPN4, YAP1, etc.)
...
GOgriddf <- group_by(GOdf, regulator, regulator_common, environment) |> 
  ...
  ungroup()
GOlist <- purrr::map2(GOgriddf$regulator, GOgriddf$environment, \(tf, e) {
...
names(GOlist) <- paste(GOgriddf$regulator_common, GOgriddf$environment, sep = "_")
```


GO analysis of remaining regulons with higher conserved proportions

```{r GO-regulons-conserved}
goslim <- goslim_full |> filter(GO_aspect == "F")
...
Archive: Counting number of genes in each plasticity group that are associated with terms belong to the combined term (the combined term was decided by visually inspecting the full GOslim results):

Looping through all the plasticity reversal categories to get GOSlim results:

```{r slimlist-all-diverged-plasticity}
...
```{r combining-terms-all-diverged-plasticity}
...
all_genes <- finaldf |> filter(experiment == "LowPi" & cer == 2 & par == 1) |> dplyr::select(gene_name) |> pull()
sum(term_genes %in% all_genes)/length(term_genes)
sum(term_genes %in% all_genes)
length(all_genes)
