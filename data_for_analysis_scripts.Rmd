...
sample_info |> dplyr::select(genotype) |> table() # should be all WT
sample_info_allele |> dplyr::select(genotype) |> table() # should be all WT
```
...
info <- sample_info |> dplyr::select(all_of(c("condition", "experiment", 
                              "time_point_num", "time_point_str"))) |> 
  unique() |> arrange(experiment, time_point_num)
info_allele <- sample_info_allele |> dplyr::select(all_of(c("condition", "experiment",
                                            "time_point_num", "time_point_str"))) |> 
  unique() |> arrange(experiment, time_point_num)
...
info |> filter(experiment == "LowN") |> dplyr::select(time_point_num) |> table()
dim(collapsed$par[,info$experiment == "HAP4"]) # HAP4 has not been reduced (no replicates)
info |> filter(experiment == "HAP4") |> dplyr::select(time_point_num) |> table()
```
```{r}
...
# parental counts for main clustering
...
# hybrid counts for clustering, for cis/trans test
# all WT samples, replicates averaged
load("data_files/Cleaned_Counts_Allele.RData")
dim(info_allele)
...
### Splitting counts into species/experiments
counts_list_allele <- vector(mode = "list", length = 0)
for (e in unique(info_allele$experiment)) {
  for (a in c("cer", "par")) {
    cts <- collapsed_allele[[a]][, info_allele$experiment == e]
    cts <- cts[apply(cts, 1, \(x) !all(is.na(x))),] # removing na genes from Heat/Cold
    counts_list_allele[[paste(a, e, sep = "_")]] <- cts
  }
}
rm(cts)
# removing genes that are lowly expressed in BOTH PARENTAL species
# using pre-low-expression filtered parental counts as reference
counts_list_parents_preFilter <- vector(mode = "list", length = 0)
for (e in unique(info$experiment)) {
  for (a in c("cer", "par")) {
    cts <- collapsed[[a]][, info$experiment == e]
    cts <- cts[apply(cts, 1, \(x) !all(is.na(x))),] 
    counts_list_parents_preFilter[[paste(a, e, sep = "_")]] <- cts
  }
}
rm(cts)
...
for (e in ExperimentNames) {
  cer_name <- paste("cer", e, sep = "_")
  par_name <- paste("par", e, sep = "_")
  low_cer <- rowMeans(counts_list_parents_preFilter[[cer_name]]) <= expr_thresh # intentionally based on parental counts, not hybrid
  low_par <- rowMeans(counts_list_parents_preFilter[[par_name]]) <= expr_thresh
  good_idxs <- !(low_cer & low_par)
  counts_list_allele[[cer_name]] <- counts_list_allele[[cer_name]][good_idxs,]
  counts_list_allele[[par_name]] <- counts_list_allele[[par_name]][good_idxs,]
}
# now experiments have different nGenes (but species from the same experiment will have the same nGenes, and should be the same length as parents above)
dim(counts_list_allele$cer_CC)
dim(counts_list_allele$par_CC)
dim(counts_list_allele$cer_Cold)
dim(counts_list_allele$par_Cold)
dim(info_allele)
# saving
save(counts_list_allele, info_allele, file = "data_files/Clustering_Counts_Allele.RData")
```
